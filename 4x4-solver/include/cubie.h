 ...
 *   CenterCube  — 24 X-centre stickers, one per center slot
 *   EdgeCube    — 24 wing-edge pieces, one per wing slot
 *   CornerCube  — 8 corners with permutation + orientation
 *
 * Face/color indices: U=0 R=1 F=2 D=3 L=4 B=5  (matches Moves U/R/F/D/L/B order)
 *
 ...
 * CenterCube
 *
 * ct[i] = face color (0–5) of the piece occupying center position i.
...
/* The 24 center facelet slots in solved-state position order. */
...
/* Two face-colors for each of the 12 edge pairs (indexed 0–11). */
...
typedef struct {
    ...
    int     centerAvail;  /* how many buffered moves are applied to center */
