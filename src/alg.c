...
static void alg_push(Alg *a, const uint8_t face, const uint8_t q, const uint8_t depth) {
...
char *alg_to_string(const Alg *a) {
    ...
    /* Worst case per token: "99Uw'" = 6 chars, plus one space separator. */
    char *s = malloc(7 * (size_t)a->len + 1);
    if (!s) die(EXIT_CODE_INTERNAL, STAGE_INTERNAL, NO_SITE, "OOM: alg_to_string");
    ...
    for (int i = 0; i < a->len; i++) {
        if (i > 0) s[pos++] = ' ';
        int d = a->m[i].depth > 0 ? a->m[i].depth : 1;
        if (d > 1) pos += snprintf(s + pos, 4, "%d", d);
...
static bool merge_pass(Alg *a) {
    ...
    for (int r = 0; r < a->len; r++) {
        Move mv = a->m[r];
        if (w > 0 && a->m[w - 1].face == mv.face && a->m[w - 1].depth == mv.depth) {
...
static bool parse_move_token(const char *tok, Move *out) {
    ...
    // optional digit prefix — only meaningful with a following 'w'
    int prefix = 0;
    if (*p >= '0' && *p <= '9') {
        char *end;
        errno = 0;
        long d = strtol(p, &end, 10);
        if (errno != 0 || d < 1 || d > 255) return false;
        prefix = (int)d;
        p = end;
    }

    // face letter (uppercase only)
    if (*p < 'A' || *p > 'Z') return false;
    const int face = face_from_char(*p++);
    if (face < 0) return false;

    // optional 'w': wide move of the form [n]Fw
    // depth = explicit prefix if given, else 2
    int depth = 1;
    if (*p == 'w') {
        depth = (prefix > 0) ? prefix : 2;
        p++;
    }
...
static int next_token(const char **pp, char *tokbuf, int bufsz) {
    ...
    if (*p == '(') {
        ...
        if (*p >= 'A' && *p <= 'Z') {
            ...
            return -1; // expected uppercase letter
        }
        if (*p == 'w')               PUSH(*p++);
        if (*p == '\'' || *p == '2') PUSH(*p++);
    }
...
bool alg_parse(const char *text, Alg *out) {
    ...
    while (ok) {
        ...
        if (tok[0] == '(') {
            ...
            alg_push(&stk[depth - 1], mv.face, mv.q, mv.depth);
        }
    }
